...
The existing `/order → wallet sign → /execute` flow, portfolio token-account reader and sell/refresh/buy rebalance engine are reused. Portfolio recognizes both providers; the selected basket targets apply to all resolved supported positions, including zero targets for assets outside that basket. Review all proposed sells. localStorage still stores only the active basket. sessionStorage stores the selected wallet name/address under `stocklana.wallet` to silently reconnect after a tab refresh; Disconnect clears it. Restricted storage or unavailable wallet authorization falls back to manual connection. Balances, private keys and transaction recovery are not persisted.
