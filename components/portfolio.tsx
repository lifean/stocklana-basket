...
export function Portfolio() {
  ...
  return <main className="basket-page"><div className="section-heading"><div><span className="eyebrow">YOUR WALLET, YOUR ASSETS</span><h1 className="portfolio-title">Your Portfolio</h1><p className="muted">{basket ? `Target strategy: ${basket.name}` : 'Supported token holdings'}</p></div><button className="button secondary" disabled={!owner || busy || unknown} onClick={() => { setManualLoading(true); setPreview(null); setError(null); setRefreshCount(n => n + 1); }}>Refresh</button></div>
    ...
    {snapshot && <><section className="panel portfolio-summary"><div><span className="eyebrow">TOTAL STOCK VALUE</span><h2>{snapshot.totalValue === null ? 'Unavailable' : money(snapshot.totalValue)}</h2>{snapshot.totalValue === null && <p className="small muted">Priced holdings: {money(snapshot.pricedValue)}. One or more stocks cannot be fully valued.</p>}</div><div><span className="eyebrow">WEIGHTED 24H TOKEN CHANGE</span><h2>{snapshot.weightedChange24h === null ? '—' : `${snapshot.weightedChange24h > 0 ? '+' : ''}${snapshot.weightedChange24h.toFixed(2)}%`}</h2><p className="small muted">Current-holdings price change, not portfolio P&amp;L.</p></div></section>
      ...
      {snapshot.registry.warnings?.map(warning => <p className="small muted" key={warning}>{warning}</p>)}
      ...
      <section className="panel portfolio-positions"><div className="table-scroll"><table><thead><tr><th>Asset / Provider</th><th>Value</th><th>Current</th><th>Target</th><th>Drift</th></tr></thead><tbody>{snapshot.positions.map(p => <tr key={p.mint}><th scope="row">{p.symbol}<small>{p.provider === 'prestocks' ? 'PreStocks' : p.provider === 'tessera' ? 'Tessera' : 'xStocks'}</small><small>{p.uiAmount.toLocaleString(undefined, { maximumFractionDigits: Math.min(p.decimals, 8) })} tokens</small><small>Liquidity: {p.liquidity === null ? 'Unavailable' : money(p.liquidity)}</small></th><td data-label="Value">{p.valueUsd === null ? 'Price unavailable' : money(p.valueUsd)}</td><td data-label="Current">{percent(p.currentWeightBps)}</td><td data-label="Target">{percent(p.targetWeightBps)}</td><td data-label="Drift">{p.driftBps === null ? '—' : `${p.driftBps > 0 ? '+' : ''}${percent(p.driftBps)}`}</td></tr>)}</tbody></table></div>{!snapshot.positions.some(p => p.tokenAmount > 0n) && <div className="empty-state"><h3>No Stock Portfolio Yet</h3><p className="muted">Choose a basket and start building your tokenized portfolio.</p><Link className="button primary" href="/">Explore Baskets</Link></div>}</section>
