...
export function WalletButton() {
  ...
  return hydrated ? <WalletControl /> : <button className="button secondary" disabled>Connect wallet</button>;
}
function WalletControl() {
  ...
  const busy = connect.isRunning || disconnect.isRunning || status === 'pending';
  return <div className="wallet-control">
    ...
    </button> : <button className="button secondary" aria-expanded={open} aria-controls="wallet-picker" disabled={busy} onClick={() => setOpen(!open)}>{busy ? 'Connecting…' : 'Connect wallet'}</button>}
