...
export default async function BasketPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!basket) notFound();
  return <main className="basket-page"><Link className="back-link" href="/">← All strategies</Link><div className="basket-heading"><span className="emoji">{basket.emoji}</span><span className="eyebrow">YOUR PORTFOLIO STARTS HERE</span><h1>{basket.name}</h1><p className="muted">{basket.description}</p></div><div className="basket-grid"><section className="panel"><span className="eyebrow">THE STRATEGY</span><h2>Target allocation</h2><dl className="target-list">{basket.assets.map((a, i) => <div key={a.symbol}><dt><span className={`asset-icon segment-${i}`}>{a.stockSymbol[0]}</span><span>{a.stockSymbol}<small>{a.symbol} · {basket.provider === 'prestocks' ? 'PreStocks exposure' : basket.provider === 'tessera' ? 'Tessera T-Token exposure' : 'Tokenized equity'}</small></span></dt><dd>{a.weightBps / 100}%</dd></div>)}</dl><p className="muted small">Each allocation is held as an individual token. You keep control of your wallet.</p></section><InvestmentForm basket={basket} /></div></main>;
}
